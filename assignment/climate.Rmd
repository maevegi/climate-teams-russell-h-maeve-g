...
The measurements are collected from NASA's Goddard Institute Institute for Space studies. The data includes the global average surface temperature from 1880 to 2022. The measurements are made as the hange in surface temrpature compared to the average from 1951 to 1980. The measurement uncertainty is Lowess smoothing, which averages the data.
- What is the resolution of the data?
The data is measured in annual mean temperature in Celsius. 
- Are their missing values? How should they be handled?
The only obvious gap in data is temperature data pre-1880. 
...
The data set represents the amount Antarctic and Greenland ice mass per year. The mass of the ice is measured in gigatons per year. The columns are year, Antarctic mass, and Greenland ice mass. The data comes from NASA's GRACE and GRACE follow-on satellites.
...
Both plots display small seasonal variations in ice mass as ice melts in the spring and reforms in the winter. However, both plots display an overall decline in ice mass. The Greenland ice mass appears to follow a fairly regular decline, while the Antarctic ice mass data has greater variation. Additionally, the rate of change for the Greenland ice is almost twice as fast as that of the Antarctic ice. 
...
This table is measuring the Global Mean Sea Level (GMSL) variation
...
#data <- xmlParse(file = "Vostok_Ice_Core.xml")
...
```{r}
vostok_co2 <-
read_table("https://www.ncei.noaa.gov/pub/data/paleo/icecore/antarctica/vostok/vostok_co2.txt",
          skip = 76,
          col_names = c("meter_depth", "year_depth", "air_age", "air_age_error", "co2_ave", "co2_sigma", "date", "no_samples"))
          arrange(vostok_co2, desc("year_depth"))
vostok_co2  
```
...
- Plot data
```{r}
#ggplot(data = vostok_co2, x = year_depth, y = air_age_error) +
#geom_line(mapping = aes(x = year_depth, y = air_age_error)) + 
